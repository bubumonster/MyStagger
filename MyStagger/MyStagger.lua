...
local function UpdateFrameInteractivity()
...
local function GetSharedMediaSoundFile()
    ...
    local key = GetDB().sharedMediaSound or DEFAULT_SHARED_MEDIA_SOUND
...
local function PlayAlertSound()
    if not db or not db.soundEnabled then
        return
    end
...
local function CreateOptionsPanel()
    ...
    local sizeEdit = MakeNumericEditBox(options, "Normal Font Size", 16, -64, 80, 10, 36, function()
        return db.fontSize
    end, SetFontSize)

    local alertSizeEdit = MakeNumericEditBox(options, "Alert Font Size", 140, -64, 80, 10, 48, function()
        return db.alertFontSize
    end, SetAlertFontSize)

    local xEdit = MakeNumericEditBox(options, "X Position", 16, -130, 80, -2000, 2000, function()
        return db.x
    end, SetXPosition)

    local yEdit = MakeNumericEditBox(options, "Y Position", 140, -130, 80, -2000, 2000, function()
        return db.y
    end, SetYPosition)
    ...
    helpText:SetText("Display: total stagger HP% and stagger HP% per second. Alerts use only HP%/s.")
